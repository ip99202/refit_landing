...
3. 아래 마이그레이션 순서대로 실행 (각각 SQL Editor에 붙여넣기 후 Run):
   - `supabase/migrations/20250307000000_create_user_info_landing.sql`
...
무료 플랜은 7일 미사용 시 프로젝트가 일시 중지된다. 매일 12:00(KST)에 `user_info_landing` 테이블을 조회하는 워크플로우로 이를 방지한다. anon 키로 요청 시 RLS에 의해 빈 결과만 반환되며, 개인정보는 노출되지 않는다.
