...
export default function LandingPage() {
  const viewerCount = Math.floor(Math.random() * 6) + 15;

  return (
    <>
      <section className="relative overflow-hidden bg-[#fffcf9] px-5 pb-0 pt-10 sm:px-8 sm:pt-14">
        ...
        <div className="relative mx-auto flex w-full max-w-[980px] flex-col items-center text-center">
          <div className="mb-8 inline-flex whitespace-nowrap rounded-full bg-[#24292db8] px-5 py-2 text-[clamp(15px,2.2vw,40px)] font-semibold leading-none text-white sm:mb-12 sm:px-8 sm:py-3">
            현재 {viewerCount}명이 보고있어요 👀
          </div>

          <h1
            className="text-[clamp(52px,10.8vw,100px)] font-black leading-[1.05] tracking-[-0.04em] text-[#222222]"
            style={{
              WebkitTextStroke: "clamp(2px,0.55vw,6px) #fff",
          ...
          <p className="mt-8 text-[clamp(24px,5vw,44px)] font-medium leading-[1.35] text-[#222222] sm:mt-10">
          ...
          <p className="mt-8 whitespace-nowrap text-[clamp(21px,5.3vw,48px)] font-semibold leading-[1.3] tracking-[-0.02em] text-[#222222] sm:mt-12">
            &ldquo;전 연애 기반 소개팅 서비스{" "}
            <span className="font-extrabold text-[#ff6f13]">리핏(re:fit)</span>&rdquo;
          </p>
        </div>

        <div className="relative mx-auto mt-12 flex w-full max-w-[1024px] flex-col items-center gap-8 sm:mt-16 sm:gap-12">
          <Image
            src={image1}
            alt="이별 이유 분석 섹션"
        ...
        <section className="-mx-5 mt-12 overflow-hidden bg-[linear-gradient(180deg,#111111_0%,#4b1d00_100%)] px-5 pb-28 pt-8 text-center text-white sm:-mx-8 sm:mt-16 sm:px-8 sm:pb-32 sm:pt-12">
          ...
          <div className="relative mx-auto w-full max-w-[920px]">
            <p className="text-[clamp(12px,1.9vw,32px)] font-light tracking-[-0.02em] text-[#d9d9d9]">
              ※ 리핏은 6월 초에 오픈 될 예정입니다.
            </p>

            <p className="mt-5 text-[clamp(24px,5.2vw,64px)] font-normal leading-[1.2] tracking-[-0.03em]">
              지금 <span className="font-bold">사전신청</span> 하는 분들께만 드리는
              <br />
              특별 프로모션!
            </p>

            <p className="mt-4 text-[clamp(44px,10vw,124px)] font-extrabold leading-[1] tracking-[-0.04em]">
              <span className="text-white">선착순 </span>
              <span className="text-[#ff6f13]">100명</span>
            </p>

            <div className="mx-auto mt-8 w-full max-w-[820px] rounded-2xl border border-[#ffc999] bg-white p-4 text-left sm:mt-10 sm:p-8">
              <div className="flex flex-col gap-3 sm:gap-4">
                {benefits.map((benefit) => (
                  <div key={benefit.id} className="flex items-center gap-3 sm:gap-4">
                    <div className="shrink-0 rounded-full bg-[#ff7014] px-3 py-1.5 text-[clamp(11px,1.9vw,22px)] font-bold leading-none text-[#fff4ec] sm:px-5 sm:py-2">
                      {benefit.id}
                    </div>
                    <p className="text-[clamp(13px,2.5vw,32px)] font-medium leading-[1.25] tracking-[-0.02em] text-[#222222]">
                      {benefit.text}
                    </p>
                  </div>
                ))}
